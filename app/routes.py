...
from app.models import Caja, MovimientoCaja, Producto, Usuario, Categoria
from app import db
...
def caja_nuevo_movimiento():
    ...
    if request.method == 'POST':
        ...
        usuario_id = 1  # Ajustar con tu sistema de autenticación
        nuevo = MovimientoCaja(caja_id=caja.id, tipo=tipo, monto=monto, descripcion=descripcion, usuario_id=usuario_id)
...
@login_required
def facturacion():
    return "Aquí va la Facturación"

@main_bp.route('/cobranzas')
@login_required
def cobranzas():
    return "Aquí va la Gestión de Cobranzas"
